...
//     }
    ...
    // ---- Session ----
    Session::Session( ClientBase* parent, const JID& callee, SessionHandler* jsh, std::string sId )
    ...
    Session::Session( ClientBase* parent, const JID& callee,
                      const Session::Jingle* jingle, SessionHandler* jsh ,std::string sId )
      : m_parent( parent ), m_state( Ended ), m_handler( jsh ), m_valid( false ), m_sid(sId)
    {

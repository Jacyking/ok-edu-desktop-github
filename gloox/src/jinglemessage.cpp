...
//  }
  ...
  for (; it2 != descList.end(); ++it2) {
    ...
    if (m == "audio") {
      m_medias.push_back(RTP::audio);
    } else if (m == "video") {
      m_medias.push_back(RTP::video);
    } else if (m == "application") {
      m_medias.push_back(RTP::application);
    }
  }
}


void JingleMessage::addMedia(RTP::Media media) {

...
OpenSSLBase::OpenSSLBase(TLSHandler *th, const std::string &server)
    : TLSBase(th, server), m_ssl(0), m_ctx(0), m_buf(0), m_bufsize(170000) {
...
bool OpenSSLBase::init(const std::string &clientKey,
  ...
  std::cout << "OPENSSL_VERSION_TEXT:" << OPENSSL_VERSION_TEXT << std::endl;
...
void OpenSSLBase::setClientCert(const std::string &clientKey,
                                const std::string &clientCerts) {
  ...
  if (!m_clientKey.empty() && !m_clientCerts.empty()) {
    ...
    if (SSL_CTX_use_RSAPrivateKey_file(m_ctx, m_clientKey.c_str(),
                                       SSL_FILETYPE_PEM) != 1) {
      // FIXME
    }
  }
...
//    std::cout << "SSL_get_error=>" << err << std::endl;
    switch (err) {
    ...
    if (!onceAgain && !m_recvBuffer.length())
      ...
      ((onceAgain || m_recvBuffer.length()) && (!m_secure || op == TLSRead)) ||
      ((op == TLSWrite) && (ret > 0)));
}
...
bool OpenSSLBase::handshake() {
  ...
  if (peer) {
    char peer_CN[256];
    X509_NAME_get_text_by_NID(X509_get_issuer_name(peer), NID_commonName,
    ...
    res = X509_check_host(peer, p.c_str(), p.length(),
                          X509_CHECK_FLAG_MULTI_LABEL_WILDCARDS, 0);
    if (res <=
        0) // 0: verification failed; -1: internal error; -2 input is malformed

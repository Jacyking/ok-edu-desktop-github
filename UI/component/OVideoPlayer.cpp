...
OVideoPlayer::OVideoPlayer(QWidget *parent) : QWidget(parent),
                                              ui(new Ui::OVideoPlayer),
                                              m_unit(1000)
{
    ui->setupUi(this);
...
void OVideoPlayer::setSource(const QUrl &url)
{
    ...
    if (!m_player_)
        return;
    m_source = url;
    if (url.isLocalFile() || url.scheme().isEmpty() || url.scheme().startsWith("qrc") || url.scheme().startsWith("avdevice")
...
void OVideoPlayer::setVolume()
{
    ...
    if (ao)
    {
        if (qAbs(int(ao->volume() / kVolumeInterval) - volumeSlider->value()) >= int(0.1 / kVolumeInterval))
        {
            ao->setVolume(v);
        }
    }

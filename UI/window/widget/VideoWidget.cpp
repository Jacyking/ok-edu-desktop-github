...
#include "UI/multimedia/Camera.h"
...
VideoOverlayBar::VideoOverlayBar(QWidget *parent, VideoWidgetConfig *config)
...
void VideoOverlayBar::showEvent(QShowEvent *e) { Q_UNUSED(e); }

/*视频覆盖层*/
VideoOverlay::VideoOverlay(QWidget *parent, VideoWidgetConfig *config)
  ...
  _bar = std::make_shared<VideoOverlayBar>(this, _config);
...
void VideoOverlay::paintEvent(QPaintEvent *event) { Q_UNUSED(event); }

void VideoOverlay::setCoin(int coin) {
  _coin = coin;
  QLabel *c = _bar->goldCount();
  if (c)
    c->setText(qsl("X%1").arg(_coin));
}
...
VideoWidget::VideoWidget(QWidget *parent, const VideoWidgetConfig &config,
                         const QString &jid)
    : QWidget(parent), _config(config), _jid(jid), _destoryed(false) {
  DEBUG_LOG(("..."));
...
VideoWidget::~VideoWidget() { DEBUG_LOG(("...")); }
...
void VideoWidget::resizeEvent(QResizeEvent *event) { Q_UNUSED(event); }

void VideoWidget::showEvent(QShowEvent *event) { Q_UNUSED(event); }

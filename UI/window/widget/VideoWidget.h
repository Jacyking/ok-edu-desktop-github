...
// #include "lib/ortc/OkRtcManager.h"
// #include "lib/ortc/renderer.h"
...
class VideoWidget : public QWidget //, public ortc::Renderer
{
  Q_OBJECT
public:
  ...
  VideoWidget();
  VideoWidget(QWidget *parent, const VideoWidgetConfig &config,
              const QString &jid);
...
public:
  VideoOverlayBar(QWidget *parent, VideoWidgetConfig *config);
  ...
  inline QLabel *goldCount() { return _goldCount.get(); }
...
class VideoOverlay : public QFrame {
  Q_OBJECT
public:
  VideoOverlay(QWidget *parent, VideoWidgetConfig *config);
  ~VideoOverlay();

  void paintEvent(QPaintEvent *event);

  PlayerWidget *wrap() { return _wrap; }

  QGraphicsDropShadowEffect *shadowEffect() { return _shadowEffect.get(); }

  QPainter *painter() { return _painter.get(); }

  inline int coin() { return _coin; }

  inline VideoOverlayBar *bar() { return _bar.get(); }
...
private:
  ...
  std::shared_ptr<VideoOverlayBar> _bar;
...
//    virtual const backend::UserJID& peerJID(){
//        return _jid;
//    }
//    virtual void setPeerJID(const backend::UserJID& jid){
//        _jid = jid;
//    }

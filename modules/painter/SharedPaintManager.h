...
class CSharedPaintManager : public QObject {
  ...
  typedef std::map<std::string, std::shared_ptr<CSharedPaintItemList>>
      ITEM_LIST_MAP;
...
public:
  ...
  const std::string &myId(void) { return myUserInfo_->userId(); }
  ...
  void close(void) {
    ...
    clearScreen(
        false); // DO NOT NOTIFY TO OTHERS.. JUST TRIGGER CLEAR SCREEN EVENT
    clearAllItems();
  ...
  void unregisterObserver(ISharedPaintEvent *obs) { observers_.remove(obs); }
...
public:
  ...
  void doLine(SmartBoard::SmartBoardDraw *draw,
              const SmartBoard::DrawLine *line);

  void doText(SmartBoard::SmartBoardDraw *draw,
              const SmartBoard::DrawText *text);

  void doFile(SmartBoard::SmartBoardDraw *draw,
              const SmartBoard::DrawFile *file);

  void doMove(SmartBoard::SmartBoardDraw *draw,
              const SmartBoard::DrawMove *move);

  void doRemove(SmartBoard::SmartBoardDraw *draw,
                const SmartBoard::DrawRemove *remove);

  std::shared_ptr<CSharedPainterScene> scene() { return _scene; }
  // Shared Paint Action
public:
  ...
  std::shared_ptr<CPaintItem> findPaintItem(const std::string &owner,
                                            ItemId itemId) {
  ...
  void clearScreen(bool sendData = true) {
    ...
    caller_.performMainThread(
        std::bind(&CSharedPaintManager::fireObserver_ClearScreen, this));
  }
...
public:
  ...
  void plabackTo(int position) { commandMngr_->playbackTo(position); }
...
public:
  void toDrawQueue(SmartBoard::SmartBoardDraw *draw);
...
private:
  ...
  void fireObserver_MovePaintItem(std::shared_ptr<CPaintItem> item, double x,
                                  double y) {
    DEBUG_LOG(("item：%1 pos:{x:%2,y:%3}")
  ...
  void fireObserver_SendingPacket(int packetId, size_t wroteBytes,
                                  size_t totalBytes) {
    //            for (std::list<ISharedPaintEvent *>::iterator it =
    //            observers_.begin(); it != observers_.end(); it++) {
  ...
  void onDraw(SmartBoard::SmartBoardDraw *);
...
signals:
  //    void draw(SmartBoard::SmartBoardDraw*);
  void receivedUrlInfo(ok::backend::FileResult);

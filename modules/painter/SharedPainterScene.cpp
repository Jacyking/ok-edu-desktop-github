...
CSharedPainterScene::CSharedPainterScene(ICanvasViewEvent *evt)
    : eventTarget_(nullptr), penClr_(Qt::black), penWidth_(2),
      textClr_(Qt::black), textWidth_(12), freezeActionFlag_(false),
      drawFlag_(false), hiqhQualityMoveItemMode_(false),
      currentZValue_(ZVALUE_NORMAL), backgroundColor_(Qt::white),
      gridLineSize_(0), lastCoverGraphicsItem_(nullptr),
      timeoutRemoveLastCoverItem_(0), lastTempBlinkShowFlag_(false),
...
void CSharedPainterScene::endOfDrawText() {
  ...
  for (int i = 0; i < list.size(); ++i) {
    ...
    if (_t_item) {
      _t_item->setTextInteractionFlags(
...
void CSharedPainterScene::onTimer() {
  if (nullptr == lastCoverGraphicsItem_ || timeoutRemoveLastCoverItem_ <= 0)
    return;
...
std::shared_ptr<CPaintItem>
CSharedPainterScene::findPaintItem(QGraphicsItem *item) {
  if (!item)
...
void CSharedPainterScene::setScaleImageFileItem(
    std::shared_ptr<CImageFileItem> image, QGraphicsPixmapItem *pixmapItem) {
  QPixmap pixmap;
...
static QPainterPath createCoveringBorderPath(int borderType,
                                             QGraphicsItem *item) {
...
void CSharedPainterScene::commonAddItem(std::shared_ptr<CPaintItem> item,
                                        ...
                                        int borderType) {
  DEBUG_LOG(("item id:%1 pos:{x:%2, y:%3}")
...
void CSharedPainterScene::updateItem(std::shared_ptr<CPaintItem> item) {
  ...
  if (!graphicsItem) {
    DEBUG_LOG_S(L_DEBUG) << "QGraphicsItem is nullptr!";
    return;
  }
  ...
  case PaintItemType::PT_TEXT: {
    ...
    if (item->isValid()) {
      ...
      DEBUG_LOG_S(L_DEBUG) << txt << "instead of:" << tItem->toPlainText();
...
void CSharedPainterScene::moveItem(std::shared_ptr<CPaintItem> item, double x,
                                   double y) {
  if (!item)
    return;

  DEBUG_LOG_S(L_INFO) << " item id:" << qstring(item->itemId())
                      << " move => pos:{" << x << ", " << y << "}";
  ...
  QGraphicsItem *gItem = findById(item->itemId());
  if (!gItem) {
    DEBUG_LOG_S(L_INFO) << "QGraphicsItem is nullptr!";
    return;
  }
...
void CSharedPainterScene::drawFile(std::shared_ptr<CFileItem> file) {
  ...
  DEBUG_LOG_S(L_INFO) << "file:" << file.get();
  ...
  if (base::Files::isImage(file->contentType())) //
  {
    // 打开图片
    m_networkManager->get(file->url(), [this, file](const QByteArray &buffer,
                                                    const QString &fileName) {
      Q_UNUSED(fileName)
      QImage pixmap;
      if (base::Images::putToPixmap(buffer, pixmap))
        return;

      std::shared_ptr<CImageItem> item =
          std::make_shared<CImageItem>(file->itemId());
      item->setPixmap(QPixmap::fromImage(pixmap));
      item->setMyItem();
      this->drawImage(item);
    });
  } else {
...
void CSharedPainterScene::drawImage(std::shared_ptr<CImageItem> image) {
  ...
  DEBUG_LOG_S(L_INFO) << "image:" << image.get();
...
void CSharedPainterScene::drawImageFile(
    std::shared_ptr<CImageFileItem> imageFile) {
...
void CSharedPainterScene::drawText(std::shared_ptr<CTextItem> text) {
  DEBUG_LOG_S(L_INFO) << "item id:" << qstring(text->itemId())
                      << "text:" << text->text();
  ...
  connect(item, &OkGraphicsTextItem::itemChanged, this,
          &CSharedPainterScene::onItemChanged);
...
void CSharedPainterScene::drawLine(std::shared_ptr<CLineItem> line_ptr) {
  ...
  if (line->pointCount() > 1) {
    ...
    if (line->isAvailablePosition())
      item->setPos(line->posX(), line->posY());

    item->setPath(painterPath);
    item->setZValue(currentZValue());
    item->setPen(QPen(line->color(), line->width(), Qt::SolidLine, Qt::RoundCap,
                      Qt::RoundJoin));
...
void CSharedPainterScene::drawBackgroundImage(
    std::shared_ptr<CBackgroundImageItem> image) {
...
void CSharedPainterScene::drawLineStart(const QPointF &pt, const QColor &clr,
                                        int width) {
...
void CSharedPainterScene::drawLineTo(const QPointF &pt1, const QPointF &pt2,
                                     const QColor &clr, int width) {
...
void CSharedPainterScene::addImageFileItem(const QPointF &pos,
                                           const QString &path) {
...
void CSharedPainterScene::addGeneralFileItem(const QPointF &pos,
                                             const QString &path) {
  std::shared_ptr<CFileItem> fileItem =
      std::make_shared<CFileItem>(CreateItemId(), (path));
...
void CSharedPainterScene::addGeneralUrlItem(
    const QPointF &pos, const backend::FileResult &urlInfo) {
  std::shared_ptr<CFileItem> fileItem =
      std::make_shared<CFileItem>(CreateItemId(), urlInfo.name);
...
void CSharedPainterScene::drawBackground(QPainter *painter,
                                         const QRectF &rect) {
...
void CSharedPainterScene::onItemMoving(std::shared_ptr<CPaintItem> item,
...
void CSharedPainterScene::onItemUpdate(std::shared_ptr<CPaintItem> item) {
  ...
  DEBUG_LOG(("item:%1").arg(qstring(item->itemId())));
  eventTarget_->onUpdateItem(item);
}

void CSharedPainterScene::onItemClipboardCopy(
...
void CSharedPainterScene::mousePressEvent(QGraphicsSceneMouseEvent *evt) {
  ...
  case painter::ToolboxType::P_PEN: {
    ...
    currLineItem_ =
        std::make_shared<CLineItem>(CreateItemId(), penClr_, penWidth_);
...
std::shared_ptr<CTextItem>
CSharedPainterScene::createTextItem(const QString &s) {
  QFont font;
...
void CSharedPainterScene::onItemChanged(
    OkGraphicsTextItem *item,                     //
    QGraphicsTextItem::GraphicsItemChange change, //
    const QVariant &value) {

  DEBUG_LOG_S(L_INFO) << "change:" << change << value;
  ...
  if (change == QGraphicsTextItem::GraphicsItemChange::ItemFlagsHaveChanged) {
    DEBUG_LOG_S(L_INFO) << "text:" << textItem->toPlainText();
...
void CSharedPainterScene::onScreenCapture(QPixmap pixmap) {
  ...
  //        DEBUG_LOG_S(L_INFO) << "filename: "<< filename;
...
void CSharedPainterScene::onClipboardPaste() {
  ...
  if (mimeData->hasImage()) {
    ...
    std::shared_ptr<CImageItem> item =
        std::make_shared<CImageItem>(CreateItemId());
...
void CSharedPainterScene::onCtrollerChecked(context::WB::WB_CTRL ctrl,
                                            bool isChecked) {
  if (ctrl == context::WB::WB_CTRL::WB) {
    _disabled = isChecked;
    if (_disabled) {
...
void CSharedPainterScene::removePaintItem(CPaintItem *item) {
  ...
  DEBUG_LOG_S(L_INFO) << "remove PaintItem id:" << qstring(item->itemId());

...
//#include <lib/office/pdf/PDFiumReaderMgr.h>
...
public:
  ...
  void setPenColor(const QColor &clr) { penClr_ = clr; }

  void setPenWidth(int width) { penWidth_ = width; }

  void setTextColor(const QColor &clr) { textClr_ = clr; }

  void setTextWidth(int width) { textWidth_ = width; }

  void updateBackground(void) { resetBackground(sceneRect()); }
...
private slots:
  ...
  void onCtrollerChecked(context::WB::WB_CTRL ctrl, bool isChecked);
...
public:
  ...
  void addGeneralUrlItem(const QPointF &pos,
                         const ok::backend::FileResult &urlInfo);
...
private:
  ...
  void drawLineTo(const QPointF &pt1, const QPointF &pt2, const QColor &clr,
                  int width);
  ...
  void commonAddItem(std::shared_ptr<CPaintItem> item,
                     QGraphicsItem *drawingItem, int borderType);

  void internalDrawGridLine(QPainter *painter, const QRectF &rect,
                            int gridLineSize);
...
signals:
  ...
  void openUrl(ok::backend::FileResult);

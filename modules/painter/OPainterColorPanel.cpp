...
OPainterColorPanel::~OPainterColorPanel() { delete ui; }
...
void OPainterColorPanel::checkColor(QToolButton *color) {
  // DEBUG_LOG_S(L_INFO) << "color:" << color;

  for (int i = 0; i < ui->layout_color->count(); i++) {
    QToolButton *clr =
        static_cast<QToolButton *>(ui->layout_color->itemAt(i)->widget());
    if (clr && clr != color)
...
void OPainterColorPanel::checkWeight(QToolButton *weight) {
  // DEBUG_LOG_S(L_INFO) << "weight:" << weight;

  for (int i = 0; i < ui->layout_weight->count(); i++) {
    QToolButton *clr =
        static_cast<QToolButton *>(ui->layout_weight->itemAt(i)->widget());
    if (clr && clr != weight)
      clr->setChecked(false);
  }
}

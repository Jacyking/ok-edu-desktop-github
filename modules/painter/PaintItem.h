...
#include "lib/smartboard/smartboarddrawfile.h"
...
public:
  ...
  virtual void moveItem(std::shared_ptr<CPaintItem> item, double x,
                        double y) = 0;
  ...
  virtual void
  drawBackgroundImage(std::shared_ptr<CBackgroundImageItem> line) = 0;
};
...
[[nodiscard]] inline static ItemId CreateItemId() {
  return base::Utils::GetUUID().toStdString();
}
...
public:
  ...
  CPaintItem(ItemId id)
      : canvas_(nullptr), object_(nullptr), mine_(false), packetId_(-1),
        wroteBytes_(0), totalBytes_(0), action_(Create),
    ...
    DEBUG_LOG_S(L_INFO) << "CPaintItem id:" << qstring(itemId())
                        << " create ptr=>" << this;
  }

  CPaintItem(const CPaintItem &item)
      : action_(Create), type_(PaintItemType::PT_NONE) {
  ...
  virtual Action action() { return action_; }

  virtual void setAction(Action action) { action_ = action; }

  virtual bool isValid() = 0;

  void setCanvas(IGluePaintCanvas *canvas) { canvas_ = canvas; }

  struct SPaintData &data() { return data_; }

  struct SPaintData &prevData() { return prevData_; }
  ...
  bool isAvailablePosition() { return data_.posSetFlag; }

  double posX() { return data_.posX; }

  double posY() { return data_.posY; }
  ...
  double scale() { return data_.scale; }
  ...
  void setItemId(ItemId itemId) { data_.itemId = prevData_.itemId = itemId; }

  ItemId itemId() const { return data_.itemId; }

  void setMyItem() { mine_ = true; }

  bool isMyItem() { return mine_; }
  ...
  size_t wroteBytes() { return wroteBytes_; }

  size_t totalBytes() { return totalBytes_; }
  ...
  static std::string serializeBasicData(const struct SPaintData &data,
...
public:
  virtual void
  deserialize(std::shared_ptr<SmartBoard::SmartBoardDraw> sbDraw) = 0;

  virtual std::shared_ptr<SmartBoard::SmartBoardDraw> serialize() = 0;

  virtual PaintItemType type() { return type_; }
  ...
  virtual void copyToClipboard(bool firstItem = true) {
    if (firstItem)
      QGuiApplication::clipboard()->setText(""); // init clipboard
  }
...
public:
  CLineItem(ItemId id) : CPaintItem(id) { type_ = (PaintItemType::PT_LINE); }

  CLineItem(ItemId id, const QColor &color, int width)
      : CPaintItem(id), clr_(color), w_(width) {
    type_ = (PaintItemType::PT_LINE);
  }

  size_t pointCount() const { return listList_.size(); }
  ...
  void setColor(QColor &color) { clr_ = color; }

  int width() const { return w_; }

  void setWidth(int width) { w_ = width; }

  void addPoint(const QPointF &pt) { listList_.push_back(pt); }

  virtual std::vector<QPointF> &points() { return listList_; }

  virtual void draw() {
    if (canvas_)
      canvas_->drawLine(
          std::static_pointer_cast<CLineItem>(shared_from_this()));
  }

  virtual bool isValid() { return true; }

  virtual void deserialize(std::shared_ptr<SmartBoard::SmartBoardDraw> draw) {
    if (!draw) {
      return;
    }

    DEBUG_LOG_S(L_DEBUG) << "draw: " << qstring(draw->id());
    ...
    // action
    switch (_item->action()) {
    case SmartBoard::Action::Create:
      action_ = Create;
      break;
    case SmartBoard::Action::Update:
      action_ = Update;
      break;
    case SmartBoard::ON:
      break;
  ...
  virtual std::shared_ptr<SmartBoard::SmartBoardDraw> serialize() {
    DEBUG_LOG_S(L_DEBUG) << "item:" << qstring(itemId());

    std::shared_ptr<SmartBoard::SmartBoardDraw> draw =
        std::make_shared<SmartBoard::SmartBoardDraw>();

    SmartBoard::PointList drawpoints;
    for (auto p : points()) {
      struct SmartBoard::Point sp;
      sp.x = p.x();
      sp.y = p.y();
      drawpoints.push_back(sp);
    };

    SmartBoard::DrawLine *line = new SmartBoard::DrawLine(itemId(), drawpoints);

    draw->addPlugin(line);

    // style
    SmartBoard::Style &style = draw->style();

    const std::string &color_ =
        ColorHelper::makeColorString(color(), ColorHelper::FORMAT::HEX)
            .toStdString();

    style.insert(std::pair<std::string, std::string>("color", color_));
    style.insert(
        std::pair<std::string, std::string>("width", std::to_string(width())));
...
public:
  CTextItem(ItemId id) : CPaintItem(id) { type_ = (PaintItemType::PT_TEXT); }

  CTextItem(ItemId id, const QString &text, const QFont &font,
            const QColor &color)
  ...
  virtual void setText(const QString &text) { text_ = text; }

  const QFont &font() const { return font_; }

  virtual void setFont(QFont font) { font_ = font; }
  ...
  virtual void draw() {
    if (canvas_) {
      canvas_->drawText(
          std::static_pointer_cast<CTextItem>(shared_from_this()));
    }
  }
  ...
  virtual bool isValid() { return !text_.isNull() && !text_.isEmpty(); }
  ...
  virtual void deserialize(std::shared_ptr<SmartBoard::SmartBoardDraw> draw) {
    ...
    const SmartBoard::DrawText *_item =
        draw->findPlugin<SmartBoard::DrawText>(SmartBoard::DrawType::Text);
  ...
  virtual std::shared_ptr<SmartBoard::SmartBoardDraw> serialize() {
    ...
    auto data = new SmartBoard::DrawText(itemId(), text_.toStdString());

    // action
    switch (action()) {
    case Create:
      data->setAction(SmartBoard::Action::Create);
      break;
    case Update:
      data->setAction(SmartBoard::Action::Update);
      break;
    }
    ...
    style.insert(std::pair<std::string, std::string>(
        "color", ColorHelper::makeColorString(color(), ColorHelper::FORMAT::HEX)
                     .toStdString()));

    style.insert(std::pair<std::string, std::string>(
        "pixel-size", std::to_string(font_.pixelSize())));

    DEBUG_LOG_S(L_DEBUG) << "draw:" << (draw.get());
...
public:
  CImageItem(ItemId id) : CPaintItem(id) { type_ = (PaintItemType::PT_IMAGE); }
  ...
  virtual void draw() {
    if (canvas_)
      canvas_->drawImage(
          std::static_pointer_cast<CImageItem>(shared_from_this()));
  }
  ...
  std::shared_ptr<SmartBoard::SmartBoardDraw> serialize() {
    return std::shared_ptr<SmartBoard::SmartBoardDraw>();
  }
...
public:
  ...
  std::shared_ptr<SmartBoard::SmartBoardDraw> serialize() const {
...
public:
  ...
  CFileItem(ItemId id, const QString &name)
      : CPaintItem(id), size_(0), name_(name) {
    type_ = PaintItemType::PT_FILE;
  }

  virtual const QString &name() const { return name_; }

  virtual const QString &url() const { return url_; }

  virtual void setContentType(const QString &contentType) {
    contentType_ = contentType;
  }

  virtual const QString &contentType() const { return contentType_; }

  virtual void setUrl(const QString &url) { url_ = url; }

  virtual void setMd5(const QString &md5) { md5_ = md5; }

  virtual base::FileContentType getContentType() const {
    return base::Files::GetContentType(name_);
  }

  virtual const QByteArray &byteArray() const { return byteArray_; }

  virtual void draw() {
    if (canvas_)
      canvas_->drawFile(
          std::static_pointer_cast<CFileItem>(shared_from_this()));
  }
  ...
  virtual void deserialize(std::shared_ptr<SmartBoard::SmartBoardDraw> draw) {
    ...
    auto _item =
        draw->findPlugin<SmartBoard::DrawFile>(SmartBoard::DrawType::File);
    if (!_item) {
      return;
    }
  ...
  std::shared_ptr<SmartBoard::SmartBoardDraw> serialize() {
    ...
    auto _item = new SmartBoard::DrawFile(itemId(), name_.toStdString());
    ...
    switch (action()) {
    case Create:
      _item->setAction(SmartBoard::Action::Create);
      break;
    case Update:
      _item->setAction(SmartBoard::Action::Update);
      break;
    }
...
public:
  CImageFileItem(ItemId id) : CFileItem(id) { type_ = (PT_IMAGE_FILE); }
  ...
  virtual void draw() {
    if (canvas_)
      canvas_->drawImageFile(
          std::static_pointer_cast<CImageFileItem>(shared_from_this()));
  }

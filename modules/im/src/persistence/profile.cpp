...
QString Profile::avatarPath(const ToxPk &owner, bool forceUnencrypted) {
  ...
  if (!encrypted || forceUnencrypted) {
    return Settings::getInstance().getSettingsDirPath() + "avatars/" +
           ownerStr + ".png";
  }

  QByteArray idData = ownerStr.toUtf8();
  QByteArray pubkeyData = core->getSelfId().getPublicKey().getByteArray();
  constexpr int hashSize = 72; // TOX_PUBLIC_KEY_SIZE;
  ...
  // crypto_generichash((uint8_t *)hash.data(), hashSize, (uint8_t
  // *)idData.data(),
  //  idData.size(), (uint8_t *)pubkeyData.data(),
  //  pubkeyData.size());
  return Settings::getInstance().getSettingsDirPath() + "avatars/" +
...
void Profile::setFriendAvatar(const ToxPk owner, QByteArray pic) {
  ...
  QByteArray avatarData;
  if (!pic.isEmpty()) {
    bool loaded = base::Images::putToImage(pic, pixmap);
    if (!loaded) {
      qWarning("loadFromData failed.");
      return;
    }
    avatarData = pic;
    emit friendAvatarSet(owner, QPixmap::fromImage(pixmap));

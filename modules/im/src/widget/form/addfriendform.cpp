...
AddFriendForm::AddFriendForm() {
  userService = std::make_unique<ok::backend::UserService>();
...
void AddFriendForm::onFriendReceipts(const QList<ok::backend::OrgStaff *> &qList) {
  ...
  for (auto item : qList) {
    if (!item->username.isEmpty()) {
      ...
      connect(form, &FriendForm::onClicked, [&](QString &username) {
...
void AddFriendForm::addFriend(const QString &idText) {
  qDebug() << "addFriend" << idText;

  ToxId friendId(idText+"@"+XMPP_SERVER_HOST);
  if (!friendId.isValid()) {
    GUI::showWarning(

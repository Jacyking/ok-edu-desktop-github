...
{
  setObjectName(qsl("Page:%1").arg(static_cast<int>(PageMenu::chat)));
...
void Widget::addFriend0(const ToxPk &friendPk) {
  core->requestFriendship(ToxId(friendPk.toString()), "请求添加好友！");
}
...
void Widget::friendRequestedTo(const ToxId &friendAddress, const QString &message){
  qDebug()<<"friendRequestedTo"<<friendAddress.getPublicKey().toString()<<message;
  auto frd = FriendList::findFriend(friendAddress.getPublicKey());
  if(frd){
    return;
  }
  emit friendRequested(friendAddress, message);
}
...
void Widget::onGroupJoined(QString groupNum, const GroupId groupId, const QString& name) {
  ...
  createGroup(groupNum, groupId, name);
}

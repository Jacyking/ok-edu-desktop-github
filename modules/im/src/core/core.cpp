...
void Core::onFriendStatus(QString friendId, Tox_User_Status status) {
  qDebug() << "onFriendStatus friendId:" << friendId;
...
void Core::onGroupRoomName(const QString groupId, const QString roomName) {
  emit groupTitleChanged(groupId, QString(), roomName);
}
...
void Core::requestFriendship(const ToxId &friendId, const QString &message) {
  ...
  qDebug() << "friendId" << fId;
  tox->sendFriendRequest(fId, message);
...
void Core::removeGroup(QString groupId) {
  QMutexLocker ml{&coreLoopLock};

  bool success = tox->removeGroup(groupId);
  if (success) {
    emit saveRequest();
...
QString Core::getFriendUsername(QString friendnumber) const {
  //  QMutexLocker ml{&coreLoopLock};
  //
  //  size_t namesize = tox_friend_get_name_size(tox.get(), friendnumber,
  //  nullptr); if (namesize == SIZE_MAX) {
  //    qWarning() << "getFriendUsername: Failed to get name size for friend "
  //               << friendnumber;
  //    return QString();
  //  }
  //

...
//       m_parent->removeStanzaExtension( ExtMUCOwner );
  ...
  void MUCRoom::send(const std::string &message, const std::string &id) {
    if (m_session && m_joined)
      m_session->send(message, id);
  }
                                                 ...
                                                 const std::string& node )
  {
    ...
    void MUCRoom::handleDiscoItemsResult(const JID &from, const Disco::ItemList &items) {

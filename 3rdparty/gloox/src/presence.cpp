...
namespace gloox
{
  ...
  const Devices* Presence::devices() const
  {
    return m_devices;
  }

  const AudioMuted* Presence::audioMuted() const
  {
    return m_audioMuted;
  }

  const VideoMuted* Presence::videoMuted() const
  {
    return m_videoMuted;
  }

  const VideoType *Presence::videoType() const
  {
    return m_videoType;
  }

  const RaisedHand *Presence::raisedHand() const{
     return m_raisedHand;
  }
}

...
void MessageProcessor::SharedParams::onUserNameSet(const QString &username) {
  ...
  nameMention =
      QRegularExpression("\\b" + QRegularExpression::escape(username) + "\\b",
                         QRegularExpression::CaseInsensitiveOption);
  sanitizedNameMention =
      QRegularExpression("\\b" + QRegularExpression::escape(sanename) + "\\b",
                         QRegularExpression::CaseInsensitiveOption);
}
 ...
 */
std::vector<Message>
MessageProcessor::processOutgoingMessage(bool isAction,
...
Message MessageProcessor::processIncomingMessage(bool isAction,
  ...
  if (detectingMentions) {
    ...
    for (auto const &mention :
         {nameMention, sanitizedNameMention, pubKeyMention}) {
      ...
      ret.metadata.push_back(
          {MessageMetadataType::selfMention, pos, pos + length});
      break;
    }
  }

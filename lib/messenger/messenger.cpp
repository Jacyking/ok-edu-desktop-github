...
//  });
  ...
  connect(_im, &IM::groupListReceived, this,
          [&](const QString& groupId, const QString& name) {
            for (auto handler : groupHandlers) {
  ...
  connect(_im, &IM::groupRoomName, this,
          [&](const QString &groupId, const QString &roomName) -> void {
            for (auto handler : groupHandlers) {
...
void Messenger::sendFriendRequest(const QString &f, const QString &message) {
  DEBUG_LOG(("sendFriendRequest friend:%1 msg:%2").arg((f)).arg(message));
  auto _session = ok::session::AuthSession::Instance();
  auto _im = _session->im();
  _im->addRosterItem(JID(f.toStdString()), message);
}
